...
import { GET_ANIME_LIST, ANIME_LOADING } from "../../actions/types";
...
function AllAnimeList(props) {
  ...
  const animeList = Array.from(useSelector(state => state.anime.animeList));

  const loading = useSelector(state => state.anime.loading);

  const animeLength = animeList.length || 0;

  for (let index = 0; index < animeLength; index++) {
    animeList[index].imageAnime = ImageData.anime.list[index];
  }

  let animeListed =
    animeList &&
    animeList.map(animeList => (
      <Fragment key={animeList._id}>
        <Col>
          <Card>
            <CardItem
              style={{
  ...
  while (i <= total) {
    children.push(animeListed.shift());
    if (i % 3 === 0) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://192.168.18.17:5000/api/anime-list/?page=${page}`)
